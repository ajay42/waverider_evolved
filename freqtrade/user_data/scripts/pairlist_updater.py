...
def load_settings() -> dict:
    ...
    return {
        ...
        # Parking: deals that can NEVER buy again (braked / draining /
        # decision-age) stop consuming a trading slot - the list grows by
        # the parked count (capped) so fresh coins can work. Refills are
        # gated: total deployment under the refill ceiling, BTC not
        # crashing, quality floor met.
        "parking_enabled": bool(wr.get("parking_enabled", False)),
        "max_parked_deals": int(wr.get("max_parked_deals", 3)),
        "parking_refill_max_exposure_pct": float(wr.get("parking_refill_max_exposure_pct", 45)),
        "parked_exclusion_cap_pct": float(wr.get("parked_exclusion_cap_pct", 0)),
        "decision_waves": float(wr.get("decision_waves", 12)),
        "wave_period_fallback_hours": float(wr.get("wave_period_fallback_hours", 4)),
        "regime_brake_btc_drop_perc": float(wr.get("regime_brake_btc_drop_perc", 5)),
        "dry_run_wallet": float(config.get("dry_run_wallet", 10000)),
...
def read_deal_counts() -> dict:
    ...
    for r in con.execute(
        "SELECT id, pair, is_open, close_date, close_profit_abs, exit_reason, "
        "open_date, stake_amount FROM trades"
    ):
        entry = result.setdefault(r["pair"], {"open": 0, "closes": [],
                                              "open_trades": []})
        if r["is_open"]:
            entry["open"] += 1
            entry["open_trades"].append({
                "id": r["id"], "open_date": r["open_date"],
                "stake": r["stake_amount"] or 0.0, "braked": False,
            })
            open_ids[r["id"]] = entry["open_trades"][-1]
        elif r["close_date"]:
            entry["closes"].append(
    ...
    if open_ids:
        ...
        for r in con.execute(
            f"SELECT ft_trade_id, cd_value FROM trade_custom_data "
            f"WHERE cd_key='coin_braked' AND ft_trade_id IN ({marks})",
            list(open_ids)):
            if str(r["cd_value"]).strip('"') in ("1", "true", "True"):
                open_ids[r["ft_trade_id"]]["braked"] = True
    con.close()
    return result
...
def run_cycle() -> None:
    ...
    # --- parking: never-buy-again deals stop consuming trading slots ---
    parked = []
    if settings["parking_enabled"]:
        for pair, joined_iso in state["active"].items():
            info = deals.get(pair, {})
            if not info.get("open_trades"):
                continue
            braked = any(t["braked"] for t in info["open_trades"])
            period = float(state.get("wave_period_hours", {}).get(pair)
                           or settings["wave_period_fallback_hours"])
            old = False
            for t in info["open_trades"]:
                opened = datetime.fromisoformat(t["open_date"].replace(" ", "T"))
                if opened.tzinfo is None:
                    opened = opened.replace(tzinfo=timezone.utc)
                age_waves = (now - opened).total_seconds() / 3600.0 / max(period, 0.1)
                if age_waves >= settings["decision_waves"]:
                    old = True
            if braked or old or pair in state["draining"]:
                parked.append(pair)
        parked = parked[:settings["max_parked_deals"]]
    state["parked"] = parked
    if parked:
        log(f"parked (not consuming trading slots): {', '.join(parked)}")

    # --- refill gates: deployment ceiling and market health ---
    refills_allowed = True
    if settings["parking_enabled"] and parked:
        total_stake = sum(t["stake"] for info in deals.values()
                          for t in info.get("open_trades", []))
        # Reserve exclusion: parked capital (capped) doesn't count against
        # the refill ceiling either - mirrors the bot's working-budget rule.
        parked_stake = sum(t["stake"] for pr in parked
                           for t in deals.get(pr, {}).get("open_trades", []))
        total_stake -= min(parked_stake,
                           settings["parked_exclusion_cap_pct"] / 100.0
                           * settings["dry_run_wallet"])
        if total_stake > settings["parking_refill_max_exposure_pct"] / 100.0                 * settings["dry_run_wallet"]:
            refills_allowed = False
            log(f"refills paused: deployment ${total_stake:.0f} over the "
                f"{settings['parking_refill_max_exposure_pct']:.0f}% refill ceiling")
        else:
            try:
                btc = http_json(f"{BINANCE}/api/v3/ticker/24hr?symbol=BTCUSDT")
                if float(btc.get("priceChangePercent", 0)) <= -abs(settings["regime_brake_btc_drop_perc"]):
                    refills_allowed = False
                    log("refills paused: BTC crashing")
            except Exception:
                pass  # no data -> keep refills allowed (bot-side gates still apply)

    # --- refill from the ranking (list grows by the parked count) ---
    target_active = settings["max_coins"] + len(parked)
    free_slots = (target_active - len(state["active"])) if refills_allowed else 0
    if free_slots > 0:
        ...
        for pair, delta in candidates:
            if free_slots == 0:
                break
            # Joiners must clear the SAME quality floor that keeps coins on
            # the list - an empty slot beats a mediocre coin. Candidates are
            # sorted best-first, so the first miss ends the refill.
            if settings["min_active_wave_score"] > 0 \
                    and delta < settings["min_active_wave_score"]:
                log(f"refill stopped: best candidate {pair} scores {delta:.2f} "
                    f"below the {settings['min_active_wave_score']} floor - "
                    f"slot stays empty")
                break
