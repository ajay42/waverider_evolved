...
class WaveRiderDCA(IStrategy):
    ...
    # Hard absolute-clock backstop: no deal may stay open longer than this many
    # days. At the cap the deal is force-closed (tag "age_cap_close"), accepting
    # any loss - the cost of the no-trap guarantee (capital safety before
    # returns). Independent of wave count; typically fires only on dead,
    # wave-less coins the lifecycle deadlines would otherwise let linger.
    # Data-driven: win rate collapses as deals age past ~12 waves; 5 days
    # (~8 waves at ~15h) caps a deal before that cliff. 0 disables.
    max_deal_age_days: float = 5.0
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float,
        ...
        if not has_open_trade:
            ...
            # Capitulation OR drain-eviction OR age-cap ends the coin's stint
            # outright (no fresh grace phase may reopen it this stint).
            if any(t.exit_reason in ("phase_d_close", "drain_close", "age_cap_close")
                   for t in closed):
                return False
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        # Hard age cap: no deal older than max_deal_age_days. Absolute-clock
        # backstop above every lifecycle deadline - a wave-less coin must not
        # trap capital indefinitely. Full close, loss accepted; mirrors the
        # drain_close full-exit path.
        if self.max_deal_age_days > 0:
            age_days = (current_time - trade.open_date_utc).total_seconds() / 86400.0
            if age_days >= self.max_deal_age_days:
                logger.info(
                    f"{trade.pair}: AGE CAP - deal open {age_days:.1f}d "
                    f">= {self.max_deal_age_days}d, force close (age_cap_close), "
                    f"loss {current_profit * 100:.2f}%")
                trade.set_custom_data("pending_skim", [f["id"] for f in fills])
                trade.set_custom_data("pending_skim_recycle_rung", 0)
                return -trade.stake_amount, "age_cap_close"
