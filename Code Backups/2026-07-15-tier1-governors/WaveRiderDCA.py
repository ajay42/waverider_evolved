...
PHASE_NAMES = ("grace", "wr", "grid", "decision")
...
class WaveRiderDCA(IStrategy):
    ...
    plot_config = {
        "main_plot": {
            "overall_avg": {"color": "#4caf50"},
            "pair_avg": {"color": "#2196f3"},
            "tp_price": {"color": "#00e5ff"},
            "next_so_price": {"color": "#ffc107"},
        },
        "subplots": {
            "phase": {
                ...
                "phase_decision": {"color": "#e91e63", "type": "bar"},
            },
        },
    }
    ...
    def _stepped_deviation_percent(self, so_index: int) -> float:
        """Cumulative % distance of SO `so_index` (1-based) from the BO price."""
        if so_index <= 0:
            return 0.0
        total = 0.0
        for step in range(1, so_index + 1):
            total += self.safety_order_price_deviation_perc * (
                self.safety_order_price_step_scale ** (step - 1)
            )
        return total

    def _so_trigger_price(self, so_index: int, base_price: float) -> float:
        dev = self._stepped_deviation_percent(so_index)
        if self.is_long:
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for col in ("overall_avg", "pair_avg", "tp_price", "next_so_price",
        ...
        if self.lifecycle_enabled:
            ...
            plan = trade.get_custom_data("phase_plan", default=None) or "wr,grid"
            try:
        ...
        if current_phase == "grace":
            ...
            combo_avg = (sum(f["price"] * f["qty"] for f in combo)
                         / sum(f["qty"] for f in combo))
            dataframe.loc[since_open, "pair_avg"] = combo_avg
    ...
    def informative_pairs(self):
        ...
        if self.regime_brake_enabled or self.crash_freeze_enabled:
            return [(self.regime_reference_pair, self.timeframe)]
        return []

    def _reference_change_24h(self, current_time: Optional[datetime] = None) -> Optional[float]:
        """Reference pair's 24h % change: live ticker, or candles in backtests."""
        if self.dp and self.dp.runmode.value in ("live", "dry_run"):
            try:
                ...
                return None
        try:
            df, _ = self.dp.get_analyzed_dataframe(self.regime_reference_pair, self.timeframe)
        except Exception:
            return None
        ...
        prev_close = float(df["close"].iloc[-candles_per_day - 1])
        return 100.0 * (now_close / prev_close - 1) if prev_close > 0 else None

    def _market_in_crash(self, current_time: Optional[datetime] = None) -> bool:
        """Regime brake: is the reference pair down past the threshold over
        24h? Fails open (no brake) if the signal can't be computed."""
        change = self._reference_change_24h(current_time)
    ...
    def _crash_frozen(self, current_time: datetime) -> bool:
        """
    ...
    def _aggregate_exposure_ok(self, additional_usd: float) -> bool:
        """
        ...
        if self.max_aggregate_exposure_pct <= 0:
            return True
        total = sum((t.stake_amount or 0.0)
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float,
        ...
        if not has_open_trade:
            ...
            if any(t.exit_reason == "phase_d_close" for t in closed):
                return False
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if self.is_long:
            tp_price = avg_price * (1 + self.take_profit_perc / 100.0)
    ...
    def _grace_check(self, trade: Trade, fills: list, current_rate: float,
        ...
        if self.is_long:
            tp_hit = current_rate >= avg_price * (1 + self.take_profit_perc / 100.0)
        else:
            tp_hit = current_rate <= avg_price * (1 - self.take_profit_perc / 100.0)
        if tp_hit:
            trade.set_custom_data("pending_skim", [f["id"] for f in fills])
    ...
    def _safety_order_check(self, trade: Trade, fills: list, current_rate: float,
        ...
        if refill_rungs and allow_refills and not self._is_draining(trade.pair):
            rung = refill_rungs[0]
            rung_price = self._so_trigger_price(rung, base_price)
            ...
            if rung_hit:
                ...
                if so_stake <= self.max_exposure_per_coin_usd - self._exposure_usd(fills) \
                        and self._aggregate_exposure_ok(so_stake):
        ...
        rung_price = self._so_trigger_price(next_so_index, base_price)
        if self.is_long:
        ...
        if so_stake > self.max_exposure_per_coin_usd - self._exposure_usd(fills):
            return None
        if not self._aggregate_exposure_ok(so_stake):
    ...
    def _phase_d_decision(self, trade: Trade, fills: list, current_rate: float,
        ...
        if loss_perc <= self.phase_d_loss_threshold_perc:
            decision, reason = "close", (
              ...
              and not self._market_in_crash(current_time)):
